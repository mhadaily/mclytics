...
export default Ember.Component.extend({
  ...
  projection: Ember.computed('datesGroup', function () {
    ...
    numDays < daysInMonth ? numDays = numDays - 0.5 : numDays;
    numDays == daysInMonth ? daysInMonth : daysInMonth = daysInMonth - 0.5;


    // return (totalAmount / Math.max(0, numDays - 0.5)) * daysInMonth;
    return (totalAmount / Math.max(0, numDays)) * daysInMonth;


  }),

  leastSquaresCoeff: Ember.computed('datesGroup', 'selectedDepartment', 'selectedMonth', function () {
    var xSeries = d3.range(1, this.get('datesGroup').length),
      ySeries = this.get('datesGroup').slice(0, -1).map(function (d) {
  ...
  totalAmount: Ember.computed('entries', 'selectedDepartment', function () {
    ...
    return this.get('entries').groupAll().reduceSum(function (d) {
      return d.amount;
    }).value();
  }),
  ...
  dates: Ember.computed('entries', function () {
    return this.get('entries').dimension(d => {
  ...
  groups: Ember.computed('entries', function () {
    return this.get('entries').dimension(d => {
      return d.product.product_group || 'No Group';
    });
  }),
